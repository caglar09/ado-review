  ...
  constructor(
    ...
    if (!personalAccessToken || personalAccessToken.trim().length === 0) {
      ...
      throw errorHandler.createUserError(
        'Missing Azure DevOps PAT. Set AZURE_DEVOPS_PAT environment variable with a valid Personal Access Token.',
        {
  ...
  public async createCommentThread(
    ...
    try {
      ...
      if (typeof threadOrFilePath === 'string') {
        ...
        const safeThread = { ...threadOrFilePath } as ADOCommentThread;
      ...
      const message = data?.message || (data?.error?.message) || (error as Error).message;
