...
import { LoadedRules, RuleSet, ProjectGuideline } from './rulesLoader';
...
export class ContextBuilder {
   ...
   */
  private createBatch(
    baseContext: { projectGuidelines: string; reviewRules: string },
    hunks: DiffHunk[],
    loadedRules: LoadedRules
  ): ReviewContext {
   ...
   */
  private buildGuidelinesSection(guidelines: ProjectGuideline[], compact: boolean = false): string {
   ...
   * Build rules section
   */
  private buildRulesSection(ruleSet: RuleSet, compact: boolean = false): string {
   ...
   * Build diffs section
   */
  private buildDiffsSection(diffHunks: DiffHunk[], compact: boolean = false): string {
   ...
   */
  private buildMetadataSection(metadata: ReviewContext['metadata']): string {
...
Return results in JSON format with the following structure:
{
   ...
   */
  private applyCustomTemplate(context: ReviewContext, template: string, includeMetadata: boolean): string {
    let result = template;
   ...
   */
  private mergeRuleSets(ruleSets: RuleSet[], prioritizeRules: string[] = []): RuleSet {
   ...
   */
  private estimateTokenCount(context: ReviewContext): number {
   ...
   * Get context summary
   */
  public getSummary(context: ReviewContext): string {
   ...
   * Validate context
   */
  public validateContext(context: ReviewContext): { valid: boolean; issues: string[] } {
    const issues: string[] = [];
