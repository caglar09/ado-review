...
export class ResultMapper {
  ...
  public mapToCommentThreads(
    ...
    try {
      ...
      const threads: ADOCommentThread[] = [];
      let mappedCount = 0;
      let skippedCount = 0;
      ...
      let summaryComment: ADOComment | undefined;
   ...
   */
  private groupByLine(findings: ReviewFinding[]): Map<string, ReviewFinding[]> {
    const grouped = new Map<string, ReviewFinding[]>();
  ...
  private createSeverityGroupThread(
    ...
    // Add header comment
    const headerContent = this.createSeverityHeaderContent(severity as any, findings.length);
    comments.push({
   ...
   */
  private createSeverityHeaderContent(severity: 'error' | 'warning' | 'info', count: number): string {
    const emoji = {
      error: '🚨',
      warning: '⚠️',
      info: 'ℹ️'
   ...
   */
  private createFindingComment(finding: ReviewFinding, options: MappingOptions): ADOComment {
    const parts: string[] = [];
   ...
   */
  public validateCommentThread(thread: ADOCommentThread): boolean {
    try {
      ...
      this.logger.debug(`Thread validation failed: ${(error as Error).message}`);
      return false;
    }
  }
   ...
   */
  public getStatistics(result: MappingResult): string {
    const { stats } = result;
